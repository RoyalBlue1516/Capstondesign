...
def train_model(data_path, model_save_path):
    ...
    results = model.train(data=data_path, epochs=100, imgsz=640, max_det=100, iou=0.5)
...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(base_path, "strawberry_dataset")  # 증강된 데이터셋 경로
    yaml_path = os.path.join(dataset_path, "dataset.yaml")  # YAML 파일 경로
    model_save_path = os.path.join(base_path, "strawberry_yolov8_model.pt")  # 학습된 모델 저장 경로
